  ...
  return (
    <main className="min-h-screen grid place-items-center p-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="card w-full max-w-md">
        ...
        <p className="text-white/70">Sign up to start generating</p>
        <form onSubmit={onSubmit} className="mt-6 space-y-4">
          <input className="input" placeholder="User name" value={name} onChange={(e)=>setName(e.target.value)} required />
          ...
          <button className="btn-accent w-full" disabled={loading}>{loading ? 'Creating...' : 'Sign up'}</button>
        </form>
        ...
        <p className="mt-4 text-sm text-white/70">Have an account? <Link href="/signin" className="underline">Sign in</Link></p>
      </div>
    </main>
  );
}
